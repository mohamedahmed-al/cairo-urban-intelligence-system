...
# 2. تحميل البيانات وبناء الشبكة (مع المسارات الصحيحة)
@st.cache_data
def load_and_build_graph():
    ...
    try:
        # قراءة البيانات من مجلد csvFiles
...
# 3. دالة توليد الخريطة التفاعلية
...
with tab1:
    ...
    if st.button("🏁 Start Algorithm Race"):
        ...
        # Dijkstra logic مع معالجة الخطأ
        with c1:
            ...
            try:
                ...
                st.error("❌ لا يوجد مسار متصل بين النقطتين.")[cite: 1]

        # A* logic مع معالجة الخطأ[cite: 1]
        with c2:
